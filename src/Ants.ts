...
const antUpgradeTexts = [
  () =>
    format(
      Decimal.pow(1.12 + 1 / 1000 * player.researches[101], Decimal.add(player.antUpgrades[1 - 1]!, G.bonusant1)),
      2
    ),
  ...
  () =>
    format(
      calculateSigmoidExponential(20, Decimal.add(player.antUpgrades[4 - 1]!, G.bonusant4).div(1000).mul(20).div(19))
        .sub(1).mul(100),
      3
    ),
  () =>
    format(
      calculateSigmoidExponential(40, Decimal.add(player.antUpgrades[5 - 1]!, G.bonusant5).div(1000).mul(40).div(39))
        .sub(1).mul(100),
      3
    ),
  ...
  () =>
    format(
      calculateSigmoidExponential(999, Decimal.add(player.antUpgrades[8 - 1]!, G.bonusant8).pow(1.1).div(10000)),
      3
    ),
...
const getAntScaling = () => {
  const start = new Decimal(6.9e7)
  const pow = new Decimal(2)
...
const getAntProdCost = (bought: DecimalSource, baseCost: DecimalSource, index: number) => {
  ...
  if (i.gte(scaling.start)) {
    i = i.div(scaling.start).root(scaling.pow).pow_base(scaling.start).sub(scaling.start).div(scaling.start.ln()).mul(
      scaling.pow
    ).add(scaling.start)
  }
...
const getAntProdTarget = (amt: Decimal, baseCost: DecimalSource, index: number) => {
  ...
  if (i.gte(scaling.start)) {
    i.sub(scaling.start).div(scaling.pow).mul(scaling.start.ln()).add(scaling.start).log(scaling.start).pow(scaling.pow)
      .mul(scaling.start)
  }
  return i
}
...
export const buyMaxProdAnts = (pos: FirstToEighth, baseCost: DecimalSource, index: number) => {
  ...
  if (
    getAntProdTarget(player[tag], baseCost, index).lt(Number.MAX_SAFE_INTEGER)
...
export const buyMaxUpgAnts = (baseCost: DecimalSource, index: number, auto: boolean) => {
  if (player.currentChallenge.ascension !== 11) {
    if (getAntUpgTarget(player.antPoints, baseCost, index).lt(Number.MAX_SAFE_INTEGER)) {
      let boughtAnt = getAntUpgTarget(player.antPoints, baseCost, index).sub(9).floor().max(
        player.antUpgrades[index - 1]!
      )
      ...
      const boughtAnt = getAntUpgTarget(player.antPoints, baseCost, index).floor().add(1).max(
        player.antUpgrades[index - 1]!
      )
      player.antUpgrades[index - 1] = boughtAnt
    }
...
export const showSacrifice = () => {
  ...
  DOMCacheGetOrSet('SacrificeMultiplier').innerHTML = i18next.t('ants.antSacMultiplier', {
    ...
    x: format(
      antSacrificePointsToMultiplier(Decimal.add(player.antSacrificePoints, sacRewards.antSacrificePoints)),
      3,
      false
    )
  })
