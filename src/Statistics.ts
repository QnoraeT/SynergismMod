...
export const loadStatisticsAccelerator = () => {
  ...
  DOMCacheGetOrSet('sA3').textContent = `+${
    format(
      Decimal.floor(Decimal.pow(Decimal.mul(G.rune1level, G.effectiveLevelMult).div(10), 1.1)),
      0,
      true
    )
  }`
  DOMCacheGetOrSet('sA4').textContent = `x${
    format(
      Decimal.mul(((G.rune1level * 1) / 200), G.effectiveLevelMult).add(1),
      3,
      true
    )
  }`
...
export const loadStatisticsMultiplier = () => {
  ...
  DOMCacheGetOrSet('sM2').textContent = `+${
    format(
          Decimal.floor(Decimal.mul(G.effectiveLevelMult, G.rune2level).div(10))
          .mul(Decimal.floor(Decimal.mul(G.effectiveLevelMult, G.rune2level).div(10).add(10)))
          .mul(50).floor().div(100),
      0,
      true
    )
  }`
  DOMCacheGetOrSet('sM3').textContent = `x${
    format(
      Decimal.mul(G.effectiveLevelMult, G.rune2level).div(200).add(1),
      3,
      true
    )
  }`
...
export const loadQuarkMultiplier = () => {
  ...
  DOMCacheGetOrSet('sGQM19').textContent = `x${
    format(
      player.singularityUpgrades.intermediatePack.level.mul(0.02) // 1.02
      .add(player.singularityUpgrades.advancedPack.level.mul(0.04)) // 1.06
      .add(player.singularityUpgrades.expertPack.level.mul(0.06)) // 1.12
      .add(player.singularityUpgrades.masterPack.level.mul(0.08)) // 1.20
      .add(player.singularityUpgrades.divinePack.level.mul(0.1)).add(1),
      3,
      true
    )
  }`
  ...
  DOMCacheGetOrSet('sGQM24').textContent = `x${
    format(
      Decimal.floor(Decimal.log10(Math.max(1, player.hepteractCrafts.quark.BAL)).add(1).mul(player.octeractUpgrades.octeractQuarkGain2.level).mul(Decimal.floor(player.octeractUpgrades.octeractQuarkGain.level.div(199))).div(1e4).add(1)),
      3,
      true
    )
  }`
...
export const loadAddCodeModifiersAndEffects = () => {
  ...
  if (Decimal.abs(Decimal.sub(addEffectStats.maxQuarks, addEffectStats.minQuarks)).gte(0.5)) {
...
export const c15RewardUpdate = () => {
  ...
  if (e.gte(exponentRequirements[0])) {
    ...
    G.challenge15Rewards[keys[0]] = Decimal.log2(e.div(175)).mul(0.02).add(1)
  }
  if (e.gte(exponentRequirements[1])) {
    // Ascension Count [1500]
    G.challenge15Rewards[keys[1]] = Decimal.log2(e.div(375)).mul(0.05).add(1)
  }
  if (e.gte(exponentRequirements[2])) {
    // Coin Exponent [3000]
    G.challenge15Rewards[keys[2]] = Decimal.log2(e.div(750)).div(150).add(1)
  }
  if (e.gte(exponentRequirements[3])) {
    // Taxes [5000]
    G.challenge15Rewards[keys[3]] = Decimal.pow(
      0.98,
      Decimal.log2(e.div(1250))
    )
  }
  if (e.gte(exponentRequirements[4])) {
    // Obtainium [7500]
    G.challenge15Rewards[keys[4]] = Decimal.pow(e.div(7500), 0.75).mul(0.2).add(1)
  }
  if (e.gte(exponentRequirements[5])) {
    // Offerings [7500]
    G.challenge15Rewards[keys[5]] = Decimal.pow(e.div(7500), 0.75).mul(0.2).add(1)
  }
  if (e.gte(exponentRequirements[6])) {
    // Accelerator Boost (Uncorruptable) [10000]
    G.challenge15Rewards[keys[6]] = Decimal.log2(e.div(2.5e3)).mul(0.05).add(1)
  }
  if (e.gte(exponentRequirements[7])) {
    // Multiplier Boost (Uncorruptable) [10000]
    G.challenge15Rewards[keys[7]] = Decimal.log2(e.div(2.5e3)).mul(0.05).add(1)
  }
  if (e.gte(exponentRequirements[8])) {
    // Rune EXP [20000]
    G.challenge15Rewards[keys[8]] = Decimal.pow(e.div(2.0e4), 1.5).add(1)
  }
  ...
  if (e.gte(exponentRequirements[13])) {
    ...
    G.challenge15Rewards[keys[13]] = Decimal.pow(
      Decimal.log2(e.div(2e5)).add(1),
      4
    )
  }
  if (e.gte(exponentRequirements[14])) {
    // Ant Bonus Levels [500k]
    G.challenge15Rewards[keys[14]] = (Decimal.ln(e.div(1.5e5)).mul(0.05)).div(Decimal.ln(2)).add(1)
  }
  if (e.gte(exponentRequirements[15])) {
    // All Cube Types III [1m]
    G.challenge15Rewards[keys[15]] = (Decimal.ln(e.div(2.5e5)).div(150)).div(Decimal.ln(2)).add(1)
  }
  if (e.gte(exponentRequirements[16])) {
    // Talisman Effectiveness [3m]
    G.challenge15Rewards[keys[16]] = (Decimal.ln(e.div(7.5e5)).mul(0.05)).div(Decimal.ln(2)).add(1)
  }
  if (e.gte(exponentRequirements[17])) {
    // Global Speed [10m]
    G.challenge15Rewards[keys[17]] = (Decimal.ln(e.div(2.5e6)).mul(0.05)).div(Decimal.ln(2)).add(1)
  }
  if (e.gte(exponentRequirements[18])) {
    // Blessing Effectiveness [30m]
    G.challenge15Rewards[keys[18]] = Decimal.pow(e.div(3e7), 0.25).mul(0.2).add(1)
  }
  if (e.gte(exponentRequirements[19])) {
    // Tesseract Building Speed [100m]
    G.challenge15Rewards[keys[19]] = Decimal.pow(e.div(1e8), 2 / 3).mul(0.2).add(1)
  }
  if (e.gte(exponentRequirements[20])) {
    // All Cube Types IV [500m]
    G.challenge15Rewards[keys[20]] = Decimal.log2(e.div(1.25e8)).div(200).add(1)
  }
  if (e.gte(exponentRequirements[21])) {
    // Spirit Effectiveness [2b]
    G.challenge15Rewards[keys[21]] = Decimal.pow(e.div(2e9), 0.25).mul(0.2).add(1)
  }
  if (e.gte(exponentRequirements[22])) {
    // Ascension Score [10b]
    G.challenge15Rewards[keys[22]] = Decimal.pow(e.div(1e10), 0.25).mul(0.25).add(1)
    if (G.challenge15Rewards[keys[22]].gte(80)) {
      G.challenge15Rewards[keys[22]] = G.challenge15Rewards[keys[22]].div(80).pow(2).mul(80)
    }
  }
  if (e.gte(exponentRequirements[23])) {
    // Quark Gain [100b]
    G.challenge15Rewards[keys[23]] = Decimal.log2(e.div(3.125e9)).div(100).add(1)
  }
  ...
  if (e.gte(exponentRequirements[33])) {
    // FREE Daily Orbs
    G.challenge15Rewards.freeOrbs = Decimal.floor(e.div(2e17).sqrt().mul(200))
  }
  if (e.gte(exponentRequirements[34])) {
    // Ascension Speed
    G.challenge15Rewards.ascensionSpeed = Decimal.log2(e.div(1.5e18)).mul(0.05).add(1.05)
  }
...
const updateDisplayC15Rewards = () => {
  ...
  DOMCacheGetOrSet('c15RequiredExponentNum').textContent = format(
    Decimal.pow(10, player.challenge15Exponent.div(challenge15ScoreMultiplier())),
    0,
    true
  )
  // dprint-ignore
...
export const gameStages = (): Stage[] => {
  const stages: Stage[] = [
    ...
    },
    {
      ...
      unlocked: player.singularityUpgrades.oneMind.level.gt(0),
      reset: player.highestSingularityCount > 0
    },
    {
      stage: 22,
      tier: 6,
      name: 'onemind-end',
      unlocked: player.singularityUpgrades.offeringAutomatic.level.gt(0),
      reset: player.highestSingularityCount > 0
    },
    {
      stage: 23,
      tier: 6,
      name: 'end-pen',
      unlocked: player.singularityUpgrades.ultimatePen.level.gt(0),
      reset: player.highestSingularityCount > 0
    },
    {
      stage: 24,
