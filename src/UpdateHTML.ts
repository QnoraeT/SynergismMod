...
export const revealStuff = () => {
  ...
  DOMCacheGetOrSet('ascHyperStats').style.display = Decimal.gt(player.challengecompletions[13], 0) ? '' : 'none'
  DOMCacheGetOrSet('ascPlatonicStats').style.display = Decimal.gt(player.challengecompletions[14], 0) ? '' : 'none'
  ...
  const automationUnlocks: Record<string, boolean> = {
    ...
    toggle41: Decimal.gt(player.challengecompletions[11], 0), // Loadouts Notifx
...
export const buttoncolorchange = () => {
  ...
  if (player.toggles[33] && player.highestSingularityCount === 0) {
    ...
    if (player.toggles[31] && player.ascensionCount.eq(0)) {
      DOMCacheGetOrSet('ascendbtn').style.boxShadow = Decimal.gt(player.challengecompletions[10], 0)
